...
# FIX #10 (part A): Pull data from session_state into plain Python variables first.
# This is needed because session_state values cannot be interpolated directly inside
# components.html() — the string is rendered as static HTML, not evaluated as Python.

# FIX #10 (part B): Guard against the page being visited before a recommendation has been made.
if len(st.session_state.get("recommended_designers", [])) < 1:
    st.warning("Please go to the Home Page and generate a recommendation first.")
    st.stop()

designer_name = st.session_state.recommended_designers[0][0]  # designers_ranked is a list of (name, score) tuples
